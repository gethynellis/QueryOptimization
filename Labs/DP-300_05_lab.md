...
3. Run a workload to generate statistics for Query Store. Navigate to the C:\Course\Query Optimization Labs\Query Performance\ExecuteRandomWorkload.sql script to execute a workload. Click execute to run the script. After execution completes, run the script a second time to generate more data. Leave the query tab open for this query.
